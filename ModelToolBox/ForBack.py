...
def univariate_backward(p_model,
                        p_features,
                        p_predictors,
                        p_X_train,
                        p_Y_train,
    ...
    for i, (feature_name, index) in enumerate(p_features):
        ...
        p_model.fit(p_X_train.iloc[:, test_index], p_Y_train, p_hyperparams)
        if p_runtime_estimate:
        ...
        # higher values imply the variable that was removed provided significant model importance
        this_marginal_result = result_full - this_result 
...
def for_back_permutations(p_model,
                          p_features,
                          p_predictors,
                          p_X_train,
                          p_Y_train,
    ...
    while keep_going and len(best_vars) > 0: # Hopefully you don't get rid of all your variables!
        ...
        df_sorted = df_agg.sort_values(by='Score', ascending=True).reset_index(drop=True)
